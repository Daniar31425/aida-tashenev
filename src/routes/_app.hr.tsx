...
import { publishVacancyToHH } from "@/lib/hhApi";
...
export function VacancyBuilder() {
  ...
  async function handleTitleBlur() {
    ...
    try {
      ...
      const out = await complete(
        `Напиши список требований для вакансии '${title}' в университете Ташенова (Шымкент, Казахстан).

Правила:
- Только профессиональные требования к опыту и навыкам
- НЕ писать про знание языков (русский, казахский, английский)
- НЕ писать раздел 'О роли' или описание должности
- НЕ завышать требования — реалистичный уровень для университета
- Опыт работы: ${exp}
${phdReq}
- Максимум 6-7 пунктов
- Формат: каждый пункт с новой строки, начинается с *
- Только на русском языке`
  ...
  async function handleSubmit() {
    if (!reqs) return toast.error("Дождитесь генерации требований");
    
    setProcessing(true);
    setPubStep(0);
    setSearchStep(0);
    setInstaStep(0);
    setButtonText("Генерирую карточку...");
    ...
    // Publish to HH.kz
    setPubStep(1);
    const hhResult = await publishVacancyToHH({
      position: title,
      requirements: reqs,
      description: reqs,
    });
    ...
    if (hhResult.success) {
      setPubStep(3);
      timeline.push({ step: "Опубликовано на hh.kz", time: format(new Date(), "HH:mm:ss", { locale: ru }) });
      log("success", `✅ Вакансия опубликована на hh.kz: ${hhResult.url}`);
    } else {
      setPubStep(2);
      log("error", `Ошибка публикации на hh.kz: ${hhResult.error}`);
      toast.error(`Ошибка публикации на hh.kz: ${hhResult.error}`);
    }

    // Candidate search (still animated for now)
    setSearchStep(1);
    await wait(800);
    setSearchStep(2);
    await wait(800);
    ...
    timeline.push({ step: "Кандидаты найдены", time: format(new Date(), "HH:mm:ss", { locale: ru }) });
    ...
    const url = hhResult.success ? hhResult.url : `hh.kz/vacancy/preview`;

    // Send Telegram notification
    try {
      await sendTelegramMessage(`
📸 <b>AIDA — Вакансия опубликована</b>

👤 <b>Должность:</b> ${title}
⏰ <b>Опыт:</b> ${exp}
🔗 <b>Instagram:</b> instagram.com/s1k4ov
🌐 <b>hh.kz:</b> ${url.startsWith('http') ? url : `https://${url}`}
